...
class StorageMock {
	static get appDataPath() {
		return path.join(APP_ROOT, 'src/app-data');
	}

	static get resourcesPath() {
		return path.join(APP_ROOT, 'src/resources');
	}
	...
	static getSampleDocPath(filename) {
		//const filename = StorageMock.getSampleDocName(sampleId);
		return path.join(APP_ROOT, 'src/public/documents/', filename);
	}

	static getPdfStampPath() {
		return path.join(APP_ROOT, 'src/public', 'PdfStamp.png');
	}
