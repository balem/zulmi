...
class Util {
	...
	static setPkiDefaults(operator) {
		...
		trustedRoots.forEach(async (root) => {
			//operator.addTrustedRootSync(StorageMock.getResourcePath(root));
			//operator.addTrustedRootSync(path.join(APP_ROOT, 'src/resources/prod.cer'));
			operator.addTrustedRootSync(path.join(APP_ROOT, 'src/resources/test.cer'));
		});
