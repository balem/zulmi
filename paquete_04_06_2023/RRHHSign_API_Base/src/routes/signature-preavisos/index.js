...
router.get('/', async (req, res, next) => {
    let query = db.pg
    .table("xml_preavisos")
    .join('employee', 'employee.id', 'xml_preavisos.employee_id')
    .select('xml_preavisos.*', 'employee.nombres', 'employee.apellidos');
...
router.get('/:id', async (req, res, next) => {
    await db.pg
    .table("xml_preavisos")
    .where('xml_preavisos.id', req.params.id)
    .join('employee', 'employee.id', 'xml_preavisos.employee_id')
    .select('xml_preavisos.*', 'employee.nombres', 'employee.apellidos')
    .then((preavisos) => {
        if (preavisos.length === 0) {
...
router.post("/new", async (req, res, next) => {
    ...
    let data ='Preaviso nueva ' + req.body.employee_id + ', ' + req.body.motivo;
        ...
        var xml =
            ...
            '<dRS xmlns="http://firma.mtess.gov.py/xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://firma.mtess.gov.py/xsd siRecepDE_v150.xsd">' +
                ...
                "<RS>" +
                    ...
                    "</encabezado>" +
                    "<cuerpo>" +
                        "<observacion>" + xmlObj.observacion + "</observacion>" +
