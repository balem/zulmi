...
router.get('/', async (req, res, next) => {
    let query = db.pg
    .table("xml_amonestaciones")
    .join('employee', 'employee.id', 'xml_amonestaciones.employee_id')
    .select('xml_amonestaciones.*', 'employee.nombres', 'employee.apellidos');

    if (req.query.fecha_amonestacion) {
        query = query.where('fecha_amonestacion', `${req.query.fecha_amonestacion}`)
    }
    ...
    .then((amonestaciones) => {
        if (amonestaciones.length === 0) {
            return res.status(200).json({
                status: "error",
                data: "Amonestaciones no encontradas"
            });
        } else {
            return res.status(200).json({
                status: "success",
...
router.get('/:id', async (req, res, next) => {
    await db.pg
    .table("xml_amonestaciones")
    .where('xml_amonestaciones.id', req.params.id)
    .join('employee', 'employee.id', 'xml_amonestaciones.employee_id')
    .select('xml_amonestaciones.*', 'employee.nombres', 'employee.apellidos')
    .then((amonestaciones) => {
        if (amonestaciones.length === 0) {
...
router.post("/new", async (req, res, next) => {
    ...
    let data ='Amonestacion nuevo ' + req.body.employee_id + ', ' + req.body.motivo;
    ...
    if (company !== false) {
        await db.pg.insert({
            ...
            fecha_amonestacion: req.body.fecha_amonestacion,
        })
            .table("xml_amonestaciones")
            .returning("*")
            .then(async xmls => {
...
router.post("/sign", async function (req, res, next) {
    try {
        //busca a empresa
        let data ='Amonestacion firma ' + req.body.user_email;
            ...
            '<dRS xmlns="http://firma.mtess.gov.py/xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://firma.mtess.gov.py/xsd siRecepDE_v150.xsd">' +
                ...
                "<RS>" +
                    ...
                    "<cuerpo>" +
                        ...
                        "<fechaAmonestacion>" + moment(xmlObj.fecha_amonestacion).format('YYYY-MM-DD') + "</fechaAmonestacion>" +
                    "</cuerpo>" +
                "</RS>" +
                "<SignatureRRHH>" +
                "</SignatureRRHH>" +
