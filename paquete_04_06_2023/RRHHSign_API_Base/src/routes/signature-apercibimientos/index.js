...
router.get('/', async (req, res, next) => {
    let query = db.pg
    .table("xml_apercibimientos")
    .join('employee', 'employee.id', 'xml_apercibimientos.employee_id')
    .select('xml_apercibimientos.*', 'employee.nombres', 'employee.apellidos');

    if (req.query.fecha_apercibimiento) {
        query = query.where('fecha_apercibimiento', `${req.query.fecha_apercibimiento}`)
    }
    ...
    query
    .then((apercibimientos) => {
        if (apercibimientos.length === 0) {
            return res.status(200).json({
                status: "error",
                data: "Apercibimientos no encontrados"
            });
        } else {
            return res.status(200).json({
                status: "success",
...
router.get('/:id', async (req, res, next) => {
    await db.pg
    .table("xml_apercibimientos")
    .where('xml_apercibimientos.id', req.params.id)
    .join('employee', 'employee.id', 'xml_apercibimientos.employee_id')
    .select('xml_apercibimientos.*', 'employee.nombres', 'employee.apellidos')
    .then((apercibimientos) => {
        if (apercibimientos.length === 0) {
            return res.status(200).json({
                status: "error",
                data: "Apercibimiento no encontrada"
            });
        } else {
            return res.status(200).json({
                status: "success",
                data: apercibimientos[0]
            });
        }
    })
})
...
router.post("/new", async (req, res, next) => {
    ...
    let data ='Apercibimiento nuevo ' + req.body.employee_id + ', ' + req.body.motivo;
    ...
    if (company !== false) {
        await db.pg.insert({
            ...
            fecha_apercibimiento: req.body.fecha_apercibimiento,
        })
            .table("xml_apercibimientos")
            .returning("*")
            .then(async xmls => {
...
router.post("/sign", async function (req, res, next) {
    try {
        //busca a empresa
        let data ='Apercibimiento firma ' + req.body.user_email;
            ...
            '<dRS xmlns="http://firma.mtess.gov.py/xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://firma.mtess.gov.py/xsd siRecepDE_v150.xsd">' +
                ...
                "<RS>" +
                    ...
                    "<cuerpo>" +
                        ...
                        "<fechaApercibimiento>" + moment(xmlObj.fecha_apercibimiento).format('YYYY-MM-DD') + "</fechaApercibimiento>" +
                    "</cuerpo>" +
                "</RS>" +
                "<SignatureRRHH>" +
                "</SignatureRRHH>" +
