...
router.post('/add-multiple', async (req, res, next) => {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        var id_profile = '';
        let errores = []
        let message = "Carga masiva de empleados";
        ...
        sheet.forEach(async data => {
            ...
            try {
                if (utils.validateEmail(data.email) === false) {
                    errores.push({ message: 'Email no valido' })
                    ...
                    await db.pg
                        ...
                        .then(async users => {
                            if (users.length > 0) {
                                errores.push({ message: 'Empleado ya registrado' })
                                console.log('empleado ya registrado')
                                ...
                                if (data.nromtesspatronal) {
                                    await db.pg
                                        ...
                                        .then(async mtess => {
                                            if (mtess.length == 0) {
                                                errores.push({ message: 'MTESS Patronal no registrado: ' + data.nromtesspatronal })
                                                        ...
                                                        .then(async users => {
                                                            ...
                                                            if ((data.departamento) && (data.departamento != '')) {
                                                                ...
                                                                if (depart.length == 0) {
                                                                    ...
                                                                    utils.insertLogs(fields.creator, message, 'Alta masiva de empleados', 'WARNING');
                                                                    await db.pg.delete().table('usuario_perfiles').where('user_id', users[0].id).then(async () => {
                                                                        await db.pg.delete().table('usuario').where('id', users[0].id).then(() => { });
                                                                    });
                                                                }
                                                            }
                                                            ...
                                                            await db.pg.transaction(async trx => {
                                                                ...
                                                                if (emailResponse.status == 'success') {
                                                                    ...
                                                                    utils.insertLogs(fields.creator, emailResponse.message + ', Email:' + data.email, 'Alta masiva de empleado: envio de correo', 'ERROR');
                                    ...
                                    utils.insertLogs(fields.creator, message, 'Carga masiva de empleados', 'WARNING');
                                }
                            }
                ...
                console.error(error);
                utils.insertLogs(fields.creator, error, 'Carga masiva de empleados', 'ERROR');
                return res.status(200).json({
                    status: "error",
