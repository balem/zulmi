...
router.post("/sign", async function(req, res, next) {
    try {
        ...
        var key = db.pg.select('ruc', 'razon_social').table('company').then(async function(values) {
            ...
            pass = await utils.replaceChar(pass);
            let fecha_xml = "";
            let mes_de_pago = "";
                            ...
                            return db.pg.select("*")
                                ...
                                .then(async companies => {
                                    ...
                                    if (employee.cert_type == 'F1') {
                                        ...
                                        let dates = await utils.verifyCertDate(`openssl/certificates/${path}/certificate.pfx`, pass);
                                        ...
                                        return db.pg.select("*")
                                            ...
                                            .then(async xmls => {
                                                ...
                                                var xml64 = xml.toString('base64');
                                                var user = 'PF-CI' + employee.identification + '-CI' + employee.identification;
                                                var signature = {
                                                    vco: companies.vco,
                                                    user: user,
                                                    pass: pass,
                                                ...
                                                var result = await axios
                                                    .post('http://hsmdigital.ddns.net/hsm/file/xml', signature)
                                                    .then(async res => {
                                                        xml = JSON.parse(res.body);
                                                        return await db.pg.update({
                                                                xml: xml
                                                            })
                                                            .table("xml")
